    ...
    def refresh_repair_history(self):
        ...
        try:
            ...
            def sort_key(row):
                ...
                if not date_str:
                    return "9999-12-31"  # 日付未入力のものは一番下に配置
                return str(date_str)

            sorted_repairs = sorted(repairs, key=sort_key)

            # 🎯 ソート済みのデータを一覧（Treeview）に追加
            for row in sorted_repairs:

                clean_values = ["" if v is None else v for v in row[1:]]
                
                # values に変換後の clean_values を渡す
                self.repair_tree.insert("", tk.END, iid=str(row[0]), values=clean_values)
